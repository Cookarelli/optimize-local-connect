...
export async function reserveFounderCategory(input: CheckoutIdentity & { categorySlug: string }, db: Firestore = getFounderFirestore()) {
  ...
  return db.runTransaction(async (transaction) => {
    ...
    transaction.set(organizationRef, { type: "vendor", name: identity.businessName, normalizedName: identity.businessName.toLowerCase(), contactName: identity.contactName, contactEmail: identity.email, contactPhone: identity.phone, ...(organizationSnapshot.exists ? {} : { createdAt: now }), updatedAt: now }, { merge: true });
    transaction.set(membershipRef, { tier: "founding_partner", organizationId, categorySlug: input.categorySlug, status: "pending", paymentSource: null, listPriceCents: FOUNDER_LIST_PRICE_CENTS, actualAmountPaidCents: null, currency: FOUNDER_CURRENCY, stripe: null, paypal: null, reservationId, reservationExpiresAt: expiresAt, ...(membershipSnapshot.exists ? {} : { createdAt: now }), updatedAt: now }, { merge: true });
...
export async function releaseFounderReservation(input: { categorySlug: string; membershipId: string; reservationId: string; eventId?: string }, db: Firestore = getFounderFirestore()) {
  ...
  return db.runTransaction(async (transaction) => {
    ...
    const occupancyRef = db.doc(`founderOccupancies/${category.reservation.organizationId}`); const occupancySnapshot = await transaction.get(occupancyRef);
...
export async function claimFounderFromVerifiedStripe(input: VerifiedStripeFounder, db: Firestore = getFounderFirestore()) {
  ...
  return db.runTransaction(async (transaction) => {
    const [eventSnapshot, categorySnapshot, membershipSnapshot, occupancySnapshot, paymentSnapshot] = await Promise.all([transaction.get(eventRef), transaction.get(categoryRef), transaction.get(membershipRef), transaction.get(occupancyRef), transaction.get(paymentRef)]);
    ...
    transaction.set(categoryRef, nextCategory); transaction.set(publicRef, publicCategory(nextCategory));
    transaction.set(membershipRef, { status: input.status, paymentSource: "stripe_paid", listPriceCents: FOUNDER_LIST_PRICE_CENTS, actualAmountPaidCents: input.actualAmountPaidCents, currency: input.currency.toUpperCase(), stripe: { customerId: input.customerId, checkoutSessionId, subscriptionId: input.subscriptionId, priceId: input.priceId }, paypal: null, currentPeriodEndsAt: Timestamp.fromDate(input.periodEnd), paidAt: Timestamp.fromDate(input.paidAt), reservationExpiresAt: null, updatedAt: now }, { merge: true });
...
export async function reconcileLegacyStripeFounder(input: LegacyStripeReconciliation, db: Firestore = getFounderFirestore()) {
  ...
  return db.runTransaction(async (transaction) => {
    ...
    transaction.set(organizationRef, { type: "vendor", name: businessName, normalizedName: businessName.toLowerCase(), contactEmail: input.customerEmail.toLowerCase(), ...(organizationSnapshot.exists ? {} : { createdAt: now }), updatedAt: now }, { merge: true });
    transaction.set(membershipRef, { tier: "founding_partner", organizationId, categorySlug: input.categorySlug, status: input.status, paymentSource: "stripe_paid", listPriceCents: FOUNDER_LIST_PRICE_CENTS, actualAmountPaidCents: input.actualAmountPaidCents, currency: input.currency.toUpperCase(), stripe: { customerId: input.customerId, checkoutSessionId: input.checkoutSessionId, subscriptionId: input.subscriptionId, priceId: input.priceId }, paypal: null, currentPeriodEndsAt: Timestamp.fromDate(input.periodEnd), paidAt: Timestamp.fromDate(input.paidAt), ...(membershipSnapshot.exists ? {} : { createdAt: now }), updatedAt: now }, { merge: true });
...
export async function manuallyGrantFounder(input: { categorySlug: string; businessName: string; organizationId?: string | null }, db: Firestore = getFounderFirestore()) {
  ...
  return db.runTransaction(async (transaction) => {
    ...
    transaction.set(organizationRef, { type: "vendor", name: businessName, normalizedName: businessName.toLowerCase(), ...(organizationSnapshot.exists ? {} : { createdAt: now }), updatedAt: now }, { merge: true });
    transaction.set(membershipRef, { tier: "founding_partner", organizationId, categorySlug: input.categorySlug, status: "manually_granted", paymentSource: "manually_granted", listPriceCents: FOUNDER_LIST_PRICE_CENTS, actualAmountPaidCents: null, currency: FOUNDER_CURRENCY, stripe: null, paypal: null, ...(membershipSnapshot.exists ? {} : { createdAt: now }), updatedAt: now }, { merge: true });
...
export async function reconcilePaypalFounder(input: { categorySlug: string; businessName: string; organizationId?: string | null; contactEmail?: string | null; paypalReferenceId: string; actualAmountPaidCents: number; currency: string; paidAt: Date }, db: Firestore = getFounderFirestore()) {
  ...
  return db.runTransaction(async (transaction) => {
    ...
    transaction.set(organizationRef, { type: "vendor", name: businessName, normalizedName: businessName.toLowerCase(), ...(input.contactEmail ? { contactEmail: input.contactEmail.toLowerCase() } : {}), ...(organizationSnapshot.exists ? {} : { createdAt: now }), updatedAt: now }, { merge: true });
    transaction.create(membershipRef, { tier: "founding_partner", organizationId, categorySlug: input.categorySlug, status: "active", paymentSource: "paypal_paid", listPriceCents: FOUNDER_LIST_PRICE_CENTS, actualAmountPaidCents: input.actualAmountPaidCents, currency: input.currency.toUpperCase(), stripe: null, paypal: { referenceId: paypalReference }, paidAt: Timestamp.fromDate(input.paidAt), createdAt: now, updatedAt: now });
    transaction.set(occupancyRef, { organizationId, categorySlug: input.categorySlug, membershipId, status: "claimed", reservationId: null, expiresAt: null, updatedAt: now }); transaction.set(categoryRef, nextCategory); transaction.set(publicRef, publicCategory(nextCategory));
