...
export function FoundingClaimForm({ seatId, industry, email, contactName }: { seatId: string; industry: string; email: string; contactName: string }) {
  ...
  return <form action={action} encType="multipart/form-data" className="grid gap-5 sm:grid-cols-2">
    ...
    <div className="sm:col-span-2"><button type="submit" disabled={pending} className="inline-flex min-h-12 w-full items-center justify-center rounded-full bg-slate-950 px-6 text-sm font-black text-white transition hover:bg-emerald-700 disabled:cursor-wait disabled:opacity-60 sm:w-auto">{pending ? "Holding your seat…" : "Continue to PayPal"}<ArrowRight className="ml-2 size-4" /></button><p className="mt-3 flex items-center gap-1.5 text-xs text-slate-500"><LockKeyhole className="size-3.5" />Payment is verified server-side. A browser success message never claims a seat.</p></div>
  </form>;
}
