...
export function FoundingPartnerOnboardingForm({ application, customerEmail, customerName }: { application: Application; customerEmail: string; customerName: string | null }) {
  ...
  return <form ref={formRef} action={action} encType="multipart/form-data" onInput={updateProgress} onChange={updateProgress} className="space-y-5">
    ...
    <Section id="services" number="Section 2 of 6" title="Services and coverage" description="Help buyers understand what you do, where you work, and when you are available." icon={MapPin}>
      <div className="grid gap-5 sm:grid-cols-2">
        <label className="text-sm font-semibold text-slate-700">Primary service category <span className="text-rose-600">*</span><select className={input} name="primaryServiceCategory" value={category} onChange={event=>setCategory(event.target.value)}><option value="">Select a category</option>{FOUNDER_CATEGORY_DISPLAY_NAMES.map(item=><option key={item} value={item}>{item}</option>)}</select><FieldError state={state} name="primaryServiceCategory" /></label>
        ...
        <div className="sm:col-span-2"><p className="text-sm font-semibold text-slate-700">Additional service categories <span className="font-normal text-slate-400">(optional)</span></p><div className="mt-2 grid max-h-64 gap-2 overflow-y-auto rounded-xl border border-slate-200 p-3 sm:grid-cols-2 lg:grid-cols-3">{FOUNDER_CATEGORY_DISPLAY_NAMES.map(category=><label key={category} className="flex items-start gap-2 rounded-lg p-2 text-xs leading-5 hover:bg-slate-50"><input type="checkbox" name="additionalServiceCategories" value={category} defaultChecked={application.additional_service_categories?.includes(category)} className="mt-0.5 size-4 shrink-0 accent-emerald-700" />{category}</label>)}</div><FieldError state={state} name="additionalServiceCategories" /></div>
    ...
    <Section id="property-manager-perk" number="Section 5 of 6" title="Connect Member Benefit" description="Feature one clear benefit for eligible Connect members, such as preferred pricing, priority scheduling, or a service-call benefit. You can change or disable it later." icon={Gift}>
      <div className="space-y-5">
        ...
        <div aria-label="Connect Member Benefit preview" className="rounded-2xl border border-amber-200 bg-amber-50 p-5"><p className="text-[10px] font-black uppercase tracking-[.14em] text-amber-700">Connect Member Benefit · Preview</p><h3 className="mt-2 text-lg font-bold text-amber-950">{perkTitle || "Your benefit title"}</h3><p className="mt-2 whitespace-pre-line text-sm leading-6 text-amber-900/75">{perkDescription || "Your clear member benefit will appear here."}</p>{perkTerms ? <p className="mt-3 border-t border-amber-200 pt-3 text-xs text-amber-900/65"><strong>Terms:</strong> {perkTerms}</p> : null}</div>
      </div>
    </Section>
    ...
    <Section id="consent" number="Section 6 of 6" title="Review and consent" description="Drafts can be saved without these confirmations. All three are required for final submission." icon={ShieldCheck}>
      <div className="space-y-3">
        ...
        <label className="flex gap-3 rounded-xl border border-slate-200 p-4 text-sm leading-6 text-slate-700"><input type="checkbox" name="termsPrivacyAccepted" defaultChecked={application.terms_privacy_accepted} className="mt-1 size-4 shrink-0 accent-emerald-700" /><span>I accept the Optimize Local Connect terms and privacy policy applicable to this Founding Member application. <span className="text-rose-600">*</span><FieldError state={state} name="termsPrivacyAccepted" /></span></label>
      </div>
    </Section>
