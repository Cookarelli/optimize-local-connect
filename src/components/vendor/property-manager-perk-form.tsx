...
export function PropertyManagerPerkForm({ eligible, category, perk }: Props) {
  ...
  return <section className="mt-8 rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
    <div className="flex items-start gap-4"><span className="grid size-11 shrink-0 place-items-center rounded-2xl bg-amber-50"><Gift className="size-5 text-amber-700" /></span><div><p className="text-xs font-black uppercase tracking-[.14em] text-amber-700">Marketplace differentiator</p><h2 className="mt-1 text-xl font-bold">Connect Member Benefit</h2><p className="mt-2 text-sm leading-6 text-slate-500">Create, replace, or disable one clear member benefit shown on your public profile.</p></div></div>
    {!eligible ? <div className="mt-6 rounded-xl bg-slate-50 p-4 text-sm text-slate-600">An active Preferred or Founding Member membership is required to publish a benefit.</div> : <form action={action} className="mt-6 grid gap-5 lg:grid-cols-[1fr_.8fr]">
      ...
      <aside className="h-fit rounded-2xl border border-amber-200 bg-amber-50 p-5"><p className="text-[10px] font-black uppercase tracking-[.14em] text-amber-700">Connect Member Benefit · Preview</p><h3 className="mt-3 text-xl font-bold text-amber-950">{title || "Your benefit title"}</h3><p className="mt-2 whitespace-pre-line text-sm leading-6 text-amber-900/75">{description || "Explain exactly what eligible Connect members receive."}</p>{terms ? <p className="mt-4 border-t border-amber-200 pt-3 text-xs text-amber-900/65"><strong>Terms:</strong> {terms}</p> : null}</aside>
    </form>}
  </section>;
}
