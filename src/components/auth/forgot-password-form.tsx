...
import { FirebaseForgotPasswordForm } from "@/src/components/auth/firebase-password-forms";

const initialState: RecoveryState = { status: "idle" };

export function ForgotPasswordForm() {
  const [state, action, pending] = useActionState(requestPasswordReset, initialState);
  if (process.env.NEXT_PUBLIC_OPERATIONAL_BACKEND === "firebase") return <FirebaseForgotPasswordForm />;
