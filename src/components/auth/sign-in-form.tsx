...
export function SignInForm({ next = "/dashboard", allowSignup = false }: { next?: string; allowSignup?: boolean }) {
  ...
  return (
    <div className="space-y-5">
      {!allowSignup ? <form action={passwordAction} className="space-y-4">
        ...
        <label htmlFor="email" className="mb-1.5 block text-sm font-medium text-slate-800">Work email</label>
        <div className="relative"><Mail aria-hidden="true" className="absolute left-3.5 top-3.5 size-4 text-slate-400"/><input id="email" name="signupEmail" type="email" autoComplete="email" required className="min-h-11 w-full rounded-xl border border-slate-200 bg-white pl-10 pr-3 text-base outline-none transition placeholder:text-slate-400 focus:border-emerald-600 focus:ring-2 focus:ring-emerald-100" placeholder="you@company.com"/></div>
      </div>}
      ...
      <form action={magicAction}>
        ...
        >
          ...
          {allowSignup ? "Email me a secure account link" : "Email me a secure sign-in link"}
        </Button>
      </form>
