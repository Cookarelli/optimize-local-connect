...
| `founding_programs` | Governed founding cohort configuration, price, currency, seat count, and hold duration. | Unique slug; active programs are publicly readable. |
| `founding_verticals` | The 25 industry categories inside the Founding Fifty. | Unique program/slug and program/order constraints. |
| `founding_seats` | Authoritative permanent numbers #001–#050 and public display state. | Unique program/number; indexed availability board; claimed seats require a vendor. |
| `founding_claims` | Complete claim and payment-outcome history, including abandoned claims. | One active hold per seat; user and admin indexes; own-claim RLS. |
