  ...
  return <main className="min-h-dvh bg-[#f7f8f4] text-slate-950">
    ...
    <section className="mx-auto grid max-w-[90rem] gap-5 px-5 py-12 sm:px-8 lg:grid-cols-3 lg:px-12 lg:py-20">{VENDOR_MEMBERSHIP_PLANS.filter((plan) => plan.publiclyPurchasable).map((plan) => {
      ...
      return <article key={plan.key} className={`flex flex-col rounded-[1.75rem] border p-7 ${founding ? "border-emerald-400 bg-emerald-50 shadow-xl shadow-emerald-950/5 lg:-translate-y-2" : "border-slate-200 bg-white"}`}>
        {founding ? <FoundingMemberAvailability ctaHref="/memberships#founder-checkout" /> : <><div className="flex items-center justify-between"><span className="grid size-11 place-items-center rounded-xl bg-emerald-50 text-emerald-700"><Sparkles className="size-5" /></span></div><h2 className="mt-7 text-2xl font-bold">{plan.name}</h2><p className="mt-4 text-4xl font-semibold">{formatVendorPlanPrice(plan)}</p><p className="mt-5 min-h-18 text-sm leading-6 opacity-75">{plan.description}</p><p className="mb-5 text-xs font-semibold opacity-75">Renews automatically each {plan.interval} unless canceled.</p><ul className="mt-6 space-y-3 border-t border-current/10 pt-5">{plan.features.map((feature) => <li key={feature} className="flex gap-2 text-sm"><Check className="mt-0.5 size-4 shrink-0 text-emerald-500" />{feature}</li>)}</ul><a href={plan.checkoutUrl} className="mt-auto inline-flex min-h-12 items-center justify-center rounded-full bg-slate-950 px-5 text-sm font-black text-white hover:bg-emerald-700">{plan.key === "preferred" ? "Join Preferred" : "Join Network"}</a></>}
      </article>;
    })}</section>
  </main>;
}
