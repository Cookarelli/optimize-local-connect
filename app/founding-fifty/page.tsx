...
function SeatCard({ seat, priceCents, isLive }: { seat: FoundingSeat; priceCents: number; isLive: boolean }) {
  ...
  return <article className={`flex min-h-60 flex-col rounded-[1.5rem] border p-5 transition ${available ? "border-emerald-200 bg-emerald-50/60 shadow-sm hover:-translate-y-0.5 hover:shadow-lg" : claimed ? "border-amber-200 bg-gradient-to-br from-amber-50 to-white" : "border-slate-200 bg-slate-50"}`}>
    ...
    {claimed ? <div className="mt-5 flex flex-1 flex-col">
      ...
      {isLive ? <Link href={`/founding-fifty/claim/${seat.id}`} className="mt-auto inline-flex min-h-11 items-center justify-center rounded-full bg-slate-950 px-4 text-sm font-bold text-white transition hover:bg-emerald-700">Claim This Seat <ArrowRight className="ml-2 size-4" /></Link> : <span className="mt-auto inline-flex min-h-11 items-center justify-center rounded-full bg-slate-200 px-4 text-center text-xs font-bold text-slate-600">Secure enrollment opening soon</span>}
...
export default async function FoundingFiftyPage() {
  ...
  return <main className="min-h-dvh bg-[#f7f8f4] text-slate-950">
    <header className="sticky top-0 z-50 border-b border-slate-200/70 bg-[#f7f8f4]/90 backdrop-blur-xl"><div className="mx-auto flex h-18 max-w-[90rem] items-center justify-between px-5 sm:px-8 lg:px-12"><Link href="/"><Logo /></Link><div className="flex items-center gap-2"><Link href="/" className="hidden min-h-11 items-center rounded-full px-4 text-sm font-semibold text-slate-600 hover:bg-white sm:inline-flex">Platform</Link><Link href="/sign-in?next=/founding-fifty" className="inline-flex min-h-11 items-center rounded-full bg-slate-950 px-5 text-sm font-bold text-white hover:bg-emerald-700">Sign in</Link></div></div></header>
