...
export default async function MarketplacePage() {
  ...
  return <div><PageHeader eyebrow="Local network" title="Verified local provider marketplace" description="Qualified local partners with structured credentials and an accountable performance history." /><div className="mt-8">{vendors.length ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{vendors.map((vendor) => <article key={vendor.organization_id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-start justify-between"><div><h2 className="font-bold text-slate-950">{vendor.organizations?.name ?? "Verified provider"}</h2><p className="mt-1 flex items-center gap-1.5 text-xs font-semibold text-emerald-700"><BadgeCheck className="size-4" /> Verified</p></div>{vendor.average_rating ? <span className="flex items-center gap-1 text-sm font-bold"><Star className="size-4 fill-amber-400 text-amber-400" /> {vendor.average_rating}</span> : null}</div><div className="mt-6 grid grid-cols-2 gap-3 border-t border-slate-100 pt-4 text-sm"><div><p className="text-xs text-slate-400">Completed jobs</p><p className="mt-1 font-semibold">{vendor.completed_job_count}</p></div><div><p className="text-xs text-slate-400">Response</p><p className="mt-1 flex items-center gap-1 font-semibold"><Clock3 className="size-3.5" />{vendor.response_time_minutes ? `${vendor.response_time_minutes} min` : "New partner"}</p></div></div></article>)}</div> : <EmptyState title="No verified providers in this market yet" description="Verified local providers will appear as the marketplace is activated." />}</div></div>;
}
