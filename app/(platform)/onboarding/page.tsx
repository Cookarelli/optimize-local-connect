...
import { createFirebasePropertyManagerOrganization } from "./actions";
...
export default async function OnboardingPage({ searchParams }: { searchParams: Promise<{ plan?: string }> }) {
  ...
  if (!plan) {
    ...
    if (isFirebaseOperationalBackend()) return <main className="min-h-dvh bg-[#f7f8f4] px-5 py-10 text-slate-950 sm:py-16"><div className="mx-auto max-w-3xl"><div className="mb-8 flex items-center justify-between"><Logo/><span className="inline-flex items-center gap-2 text-xs font-bold text-slate-500"><LockKeyhole className="size-4"/>Private Firebase onboarding</span></div><section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm sm:p-10"><span className="grid size-12 place-items-center rounded-2xl bg-emerald-50"><Building2 className="size-6 text-emerald-700"/></span><p className="mt-7 text-xs font-black uppercase tracking-[.16em] text-emerald-700">Property-management launch</p><h1 className="mt-3 text-3xl font-semibold tracking-[-.04em] sm:text-5xl">Start with your first property.</h1><p className="mt-4 max-w-2xl text-sm leading-7 text-slate-600">Create your private property-management organization, add a property, then use Request Once to send one service need through review, assignment, and completion.</p><form action={createFirebasePropertyManagerOrganization} className="mt-7 grid max-w-xl gap-3 sm:grid-cols-[1fr_auto]"><label className="sr-only" htmlFor="organizationName">Organization name</label><input id="organizationName" name="organizationName" required minLength={2} maxLength={160} placeholder="Property-management organization name" className="min-h-12 rounded-xl border border-slate-200 px-4 text-base"/><button className="min-h-12 rounded-xl bg-slate-950 px-6 text-sm font-bold text-white">Create organization</button></form><p className="mt-4 text-xs leading-5 text-slate-500">Joining an existing organization? Ask its owner or admin for a secure invitation link.</p><Link href="/pricing" className="mt-7 inline-flex min-h-11 items-center rounded-full border border-slate-200 px-5 text-sm font-bold text-slate-700">I’m a service business</Link></section></div></main>;
