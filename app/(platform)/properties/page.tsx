...
export default async function PropertiesPage() {
  ...
  if (membership && isFirebaseOperationalBackend()) {
    const properties = await listFirebaseProperties(user, membership.organizationId);
    const allowed = can(user, "properties:create", membership.organizationId);
    return <div><PageHeader eyebrow="Property Management" title="Properties" description="Private properties available for service requests and Connect Match." action={allowed?<Link href="/properties/new" className="inline-flex min-h-11 items-center rounded-full bg-slate-950 px-5 text-sm font-semibold text-white"><Plus className="mr-2 size-4"/>Add property</Link>:undefined}/><div className="mt-8">{properties.length?<div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{properties.map(property=><article key={property.id} className="rounded-2xl border border-slate-200 bg-white p-5"><h2 className="font-bold">{String(property.name)}</h2><p className="mt-2 flex gap-2 text-sm text-slate-500"><MapPin className="size-4"/>{String((property.address as {line1:string}).line1)}</p><p className="mt-4 text-xs font-bold capitalize text-emerald-700">{String(property.status)}</p></article>)}</div>:<EmptyState title="Add your first property" description="Your property stays private and will be ready for Request Once." action={allowed?<Link href="/properties/new" className="inline-flex min-h-11 items-center rounded-full bg-slate-950 px-5 text-sm font-semibold text-white">Add your first property</Link>:undefined}/>}</div></div>;
  }
