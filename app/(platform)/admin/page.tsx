...
export default async function AdminDashboardPage() {
  ...
  return <div><p className="text-sm font-semibold text-emerald-700">Platform administration</p><h1 className="mt-1 text-3xl font-semibold tracking-[-.035em] text-slate-950 sm:text-4xl">Connect control center</h1><p className="mt-2 text-sm text-slate-500">Cross-organization, industry vertical, and marketplace oversight.</p><section className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{metrics.map(([label, value, Icon]) => <article key={label} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex justify-between"><div><p className="text-sm text-slate-500">{label}</p><p className="mt-3 text-3xl font-bold">{value.toLocaleString()}</p></div><span className="grid size-10 place-items-center rounded-xl bg-emerald-50"><Icon className="size-5 text-emerald-700" /></span></div></article>)}</section><div className="mt-8 grid gap-4 lg:grid-cols-2"><Link href="/admin/impact" className="flex items-center justify-between rounded-2xl bg-emerald-700 p-6 text-white transition hover:bg-emerald-800"><span className="flex items-center gap-4"><span className="grid size-11 place-items-center rounded-xl bg-white/15"><TrendingUp className="size-5"/></span><span><span className="block font-bold">Impact Engine reporting</span><span className="mt-1 block text-sm text-emerald-100">Savings, response, jobs, vendor growth, and local spending</span></span></span><ArrowRight className="size-5"/></Link><Link href="/admin/founding-fifty" className="flex items-center justify-between rounded-2xl bg-slate-950 p-6 text-white transition hover:bg-emerald-800"><span className="flex items-center gap-4"><span className="grid size-11 place-items-center rounded-xl bg-amber-300 text-amber-950"><Award className="size-5" /></span><span><span className="block font-bold">Manage the Founding Fifty</span><span className="mt-1 block text-sm text-slate-400">Seats, payments, reservations, member display, revenue, and exports</span></span></span><ArrowRight className="size-5" /></Link></div></div>;
}
