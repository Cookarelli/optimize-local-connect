...
const cards=[
  ...
  ["Vendor Network","Review Firebase vendor profiles, eligibility, and publication.","/admin/vendor-network",Users],
...
export default async function AdminDashboardPage(){const user=await requireUser();const membership=user.memberships[0];if(!user.isSuperAdmin){if(!membership||!["owner","admin"].includes(membership.role)||membership.organizationType!=="property_management")redirect(getRoleHome(user));return <PropertyOperationsDashboard user={user} membership={membership} mode="admin"/>;}let requests=0;let vendors=0;if(isFirebaseOperationalBackend()){const db=getPlatformFirestore();const [requestSnapshot,vendorSnapshot]=await Promise.all([db.collection("serviceRequests").count().get(),db.collection("vendorProfiles").count().get()]);requests=requestSnapshot.data().count;vendors=vendorSnapshot.data().count;}else{const db=await createSupabaseServerClient();const [requestResult,vendorResult]=await Promise.all([db.from("property_manager_service_requests").select("id",{count:"exact",head:true}),db.from("vendor_profiles").select("organization_id",{count:"exact",head:true})]);requests=requestResult.count??0;vendors=vendorResult.count??0;}return <div><p className="text-sm font-semibold text-emerald-700">Platform administration</p><h1 className="mt-1 text-4xl font-semibold tracking-tight">Platform Control Center</h1><p className="mt-3 text-sm text-slate-500">Secure cross-platform control for Optimize Local Connect.</p><div className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{cards.map(([title,copy,href,Icon])=><Link key={href} href={href} className="rounded-2xl border border-slate-200 bg-white p-6 hover:border-emerald-300"><Icon className="size-6 text-emerald-700"/><h2 className="mt-5 font-bold">{title}</h2><p className="mt-2 text-sm text-slate-500">{copy}</p>{title==="Service Requests"?<p className="mt-3 text-xs font-bold text-emerald-700">{requests} current records</p>:title==="Vendor Network"?<p className="mt-3 text-xs font-bold text-emerald-700">{vendors} vendor profiles</p>:null}<span className="mt-5 inline-flex items-center text-sm font-bold text-slate-950">Open <ArrowRight className="ml-2 size-4"/></span></Link>)}</div></div>}
