...
export const metadata: Metadata = { title: "Complete your Founding Member profile", robots: { index: false, follow: false } };
  ...
  return <main className="min-h-dvh bg-[#f7f8f4] text-slate-950">
    ...
    <div className="mx-auto grid max-w-[90rem] gap-8 px-5 py-8 sm:px-8 sm:py-12 lg:grid-cols-[18rem_minmax(0,1fr)] lg:px-12">
      <aside className="h-fit space-y-5 lg:sticky lg:top-6">
        <section className="rounded-[1.5rem] bg-slate-950 p-6 text-white"><ShieldCheck aria-hidden="true" className="size-7 text-emerald-400" /><p className="mt-7 text-xs font-black uppercase tracking-[.16em] text-emerald-400">Payment confirmed</p><p className="mt-3 text-3xl font-semibold tracking-[-.05em]">{new Intl.NumberFormat("en-US", { style: "currency", currency: payment.currency }).format(payment.amount_paid_cents / 100)}</p><p className="mt-1 text-xs text-slate-400">Verified Founding Member payment</p><div className="mt-6 border-t border-white/10 pt-5"><p className="text-sm font-bold">{payment.customer_name ?? "Founding Member"}</p><p className="mt-1 break-all text-xs text-slate-400">{payment.customer_email}</p></div></section>
