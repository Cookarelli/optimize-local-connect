...
export const metadata: Metadata = { title: "Legacy Founder payment status", robots: { index: false, follow: false } };
...
export default async function FoundingPartnerSuccessPage({ searchParams }: { searchParams: Promise<{ session_id?: string }> }) {
  ...
  if (sessionId.success) {
    try {
      ...
      const { data, error } = await admin.from("founding_partner_payments")
      ...
      payment = data;
      if (payment) {
        const { data: onboardingData, error: onboardingError } = await admin.from("founding_partner_onboardings").select("status").eq("payment_id", payment.id).maybeSingle();
        if (onboardingError) throw onboardingError;
        onboarding = onboardingData;
      }
    } catch (error) {
      lookupFailed = true;
      console.error("founding_partner_payment_lookup_failed", { errorType: error instanceof Error ? error.name : "UnknownError" });
    }
  }
  ...
  return <main className="grid min-h-dvh place-items-center bg-[#f7f8f4] px-5 py-12 text-slate-950">
    <div className="w-full max-w-2xl">
      ...
      <section className="rounded-[2rem] border border-slate-200 bg-white p-7 text-center shadow-xl shadow-slate-950/5 sm:p-12">
        {paid ? <CheckCircle2 aria-hidden="true" className="mx-auto size-14 text-emerald-600" /> : invalid || lookupFailed ? <XCircle aria-hidden="true" className="mx-auto size-14 text-rose-600" /> : <Clock3 aria-hidden="true" className="mx-auto size-14 text-amber-600" />}
        ...
        <p className="mx-auto mt-5 max-w-xl text-sm leading-7 text-slate-600">{paid
          ...
          : invalid
            ...
            : lookupFailed
              ? "Your payment status was not changed. Please try again shortly; if payment completed, Stripe's verified notification remains the source of truth."
