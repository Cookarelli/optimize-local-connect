    ...
    <main className="min-h-dvh bg-[#f7f8f4] text-slate-950">
          ...
          <div className="flex flex-col gap-9 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl">
              ...
              <div className="mt-6 flex flex-wrap items-center gap-3 text-sm font-semibold text-slate-300">
                <span className="inline-flex items-center gap-2 rounded-full border border-amber-300/30 bg-amber-300/10 px-3 py-1.5 text-amber-200"><img src="/brand/optimize-local-connect/founder-vendor.png" alt="" className="size-5 object-contain" />Founding Member #4</span>
      ...
      <section className="border-y border-emerald-900/10 bg-[#e3f4e9] py-16 sm:py-20">
        <div className="mx-auto grid max-w-[90rem] gap-10 px-5 sm:px-8 lg:grid-cols-[.85fr_1.15fr] lg:items-center lg:px-12">
          <div><p className="section-kicker">Local property manager network</p><h2 className="section-title">Be visible where local work begins.</h2><p className="section-copy">Property managers can discover, save, and contact qualified vendors when they need dependable help for their buildings.</p></div>
      ...
      <section className="px-5 pb-5 sm:px-8 sm:pb-8 lg:px-12 lg:pb-12"><div className="mx-auto max-w-[90rem] rounded-[2rem] bg-emerald-700 px-7 py-14 text-white sm:px-12 sm:py-18"><div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between"><div className="max-w-3xl"><p className="text-xs font-black uppercase tracking-[.18em] text-emerald-200">Ready for your own profile?</p><h2 className="mt-4 text-4xl font-semibold tracking-[-.055em] sm:text-6xl">Founding Member — {formatVendorPlanPrice(FOUNDING_PARTNER_PLAN)}</h2><p className="mt-5 text-base leading-7 text-emerald-100">{FOUNDING_MEMBER_OFFER.monthlyComparison} Join the original network with governed recognition, reviewed visibility, and member tools. Membership does not promise leads, jobs, or revenue.</p></div><div className="flex flex-col gap-3 sm:flex-row"><Link href="/founders" className="inline-flex min-h-12 items-center justify-center rounded-full bg-white px-6 text-sm font-black text-emerald-800 hover:bg-emerald-50">{FOUNDING_MEMBER_OFFER.cta}</Link><Link href="/memberships#details" className="inline-flex min-h-12 items-center justify-center rounded-full border border-white/30 px-6 text-sm font-black text-white hover:bg-white/10">See What&apos;s Included</Link></div></div></div></section>
    </main>
  );
}
