...
export default async function SignInPage({ searchParams }: { searchParams: Promise<{ next?: string; error?: string }> }) {
  ...
  const firebaseSignup = isFirebaseOperationalBackend();
  return (
      ...
      <section className="flex items-center justify-center p-5 sm:p-10">
        <div className="w-full max-w-md">
          ...
          <div className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-[0_24px_70px_rgba(15,23,42,.08)] sm:p-9">
            ...
            <p className="mb-7 mt-2 text-sm leading-6 text-slate-500">{vendorEnrollment ? "Use a verified account before creating the vendor organization and opening Checkout." : firebaseSignup ? "Create a verified account to start a property-management organization, join an invitation, or enroll a service business." : "Access is invite-only for verified property teams and service partners."}</p>
            <SignInForm next={params.next} allowSignup={vendorEnrollment || firebaseSignup} />
