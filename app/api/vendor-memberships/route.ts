...
export async function GET() {
  ...
  const current = membership?.organizationType === "vendor"
    ? await supabase.from("vendor_memberships").select("id,status,starts_at,current_period_ends_at,next_billing_at,renewal_amount_cents,cancel_at_period_end,entitlements_snapshot,vendor_membership_levels(code,name)").eq("vendor_organization_id",membership.organizationId).order("created_at",{ascending:false}).limit(1).maybeSingle()
